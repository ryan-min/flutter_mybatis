...
class SqlStatement {
  ...
  ///
  /// `timeout="0"` means no limit, as it does for JDBC's `setQueryTimeout`,
  /// which is what MyBatis passes it to. It overrides the global default
  /// rather than falling back to it.
  Duration? get timeoutDuration {
    final seconds = timeout;
    if (seconds == null) return MybatisConfig.defaultStatementTimeout;
...
class WhereElement extends SqlElement {
  ...
  String build(Map<String, dynamic> params) {
    ...
    // MyBatis strips AND/OR followed by any whitespace, not just a space.
    // Its prefix list carries a tab/newline/carriage-return variant of each
    // word, which is why writing the AND on its own line works there.
    // Matching only on a space left `WHERE AND ID = ?`.
    final result = _stripLeadingConjunction(content);

    return 'WHERE $result';
  }
}

/// Removes a leading `AND`/`OR` that is followed by whitespace.
///
/// Matching on whitespace rather than on a single space is what MyBatis does,
/// and it is why `AND` on its own line works there.
String _stripLeadingConjunction(String content) {
  const words = ['AND', 'OR'];
  final upper = content.toUpperCase();
  for (final word in words) {
    if (!upper.startsWith(word)) continue;
    if (content.length <= word.length) continue;
    // ORDER_ID and ANDROID must not lose their first letters.
    if (content[word.length].trim().isNotEmpty) continue;
    return content.substring(word.length).trimLeft();
  }
  return content;
}
