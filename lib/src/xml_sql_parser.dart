...
class XmlSqlParser {
  ...
  static MapperConfig parse(String xmlContent) {
    try {
      ...
      for (final element in mapper.findElements('sql')) {
        ...
        fragments['$namespace.$id'] = _parseElements(element.children);
      }
      ...
      for (final tagName in ['select', 'insert', 'update', 'delete']) {
        for (final element in mapper.findElements(tagName)) {
          ...
          statements['$namespace.$id'] = statement;
        }
      }
  ...
  static SelectKeyStatement? _parseSelectKey(XmlElement parent) {
    ...
    final order = (element.getAttribute('order') ?? 'AFTER').toUpperCase();
  ...
  static SqlElement _parseElement(XmlElement element) {
    switch (element.name.local) {
      ...
      default:
        return TextElement(element.innerText.trim());
    }
  }
}
...
String substituteDollar(String text, Map<String, dynamic> params) {
  ...
  return text.replaceAllMapped(RegExp(r'\$\{([\w.]+)\}'), (match) {
    ...
    if (!params.containsKey(name)) {
      MybatisLogger.warn('No value for \${$name}; left as-is');
...
class ForeachElement extends SqlElement {
  ...
  String build(Map<String, dynamic> params) {
    ...
    final idx = index;
    final source = params[collection];
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      // children의 #{item} 을 #{__frch_item_i} 로 치환
      var itemSql = children.map((e) => e.build(params)).join(' ');
      itemSql = itemSql.replaceAll('#{$item}', '#{$indexedName}');
      if (idx != null) {
        itemSql = itemSql.replaceAll('#{$idx}', '#{__frch_${idx}_$i}');
      }
  ...
  String build(Map<String, dynamic> params) {
    ...
    if (prefixOverrides.isNotEmpty) {
      for (final override in prefixOverrides.split('|')) {
        ...
        if (content.toUpperCase().startsWith(trimmed.toUpperCase())) {
          content = content.substring(trimmed.length).trim();
          break;
        }
      }
    }
    ...
    if (suffixOverrides.isNotEmpty) {
      for (final override in suffixOverrides.split('|')) {
        final trimmed = override.trim();
        if (content.toUpperCase().endsWith(trimmed.toUpperCase())) {
          content = content.substring(0, content.length - trimmed.length).trim();
          break;
        }
      }
    }
...
class BindElement extends SqlElement {
  ...
  @override
  String build(Map<String, dynamic> params) {
    // LIKE 패턴 처리: '%' + #{param} + '%' → %value%
    final paramMatch = RegExp(r'#\{(\w+)\}').firstMatch(value);
    if (paramMatch != null) {
      final paramName = paramMatch.group(1)!;
      final paramValue = params[paramName]?.toString() ?? '';
      params[name] = value
...
///
/// A failure evaluates to `false` with a warning by default; set
/// [MybatisConfig.strictExpressions] to throw instead.
