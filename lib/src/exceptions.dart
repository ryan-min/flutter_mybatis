...
/// Thrown when a statement did not finish within its `timeout`.
///
/// **The statement was not cancelled.** JDBC lets MyBatis cancel a running
/// statement; sqflite has no equivalent, so this only stops waiting. The
/// database may still be running the statement, and it may still commit.
///
/// For a read that is harmless. For an `insert`, `update` or `delete` it means
/// the outcome is unknown: retrying can apply the write twice. Check the row
/// before retrying, or make the statement idempotent.
///
/// Implements [TimeoutException], so existing `on TimeoutException` handlers
/// keep working.
class SqlTimeoutException implements TimeoutException {
  /// The statement that timed out, as `namespace.id`.
  final String statementId;

  @override
  final Duration? duration;

  /// Creates a timeout failure for [statementId] after [duration].
  SqlTimeoutException(this.statementId, this.duration);

  @override
  String get message =>
      'statement "$statementId" did not finish within ${duration?.inMilliseconds}ms. '
      'It was NOT cancelled and may still complete, so a write may already have '
      'been applied - do not blindly retry.';

  @override
  String toString() => 'SqlTimeoutException: $message';
}
