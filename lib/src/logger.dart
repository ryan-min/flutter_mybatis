...
class MybatisLogger {
  ...
  static final Set<String> _warnedOnce = <String>{};

  /// Emits [message] at most once per process, for warnings raised on a path
  /// that runs per statement build. Repeating them every query would flood
  /// release logs without adding information.
  static void configWarningOnce(String message) {
    if (!_warnedOnce.add(message)) return;
    configWarning(message);
  }

  /// Forgets which one-shot warnings have been emitted. For tests.
  static void resetWarnings() => _warnedOnce.clear();

  /// Reports a configuration problem found while loading or building SQL.
  ///
  /// Goes to the registered [MybatisLogHandler] if there is one, so an app can
  /// route it into its own logging; otherwise it prints, because a silent
  /// misconfiguration is the thing this is meant to prevent.
