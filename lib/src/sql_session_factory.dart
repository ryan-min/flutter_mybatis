...
class SqlSessionFactory {
  ...
  void _loadMapperFromString(String xmlContent, String source) {
    ...
    for (final entry in config.statements.entries) {
      if (_allStatements.containsKey(entry.key)) {
        throw MapperLoadException(
          'duplicate statement id across mappers: ${entry.key}',
          path: source,
        );
      }
    ...
    for (final entry in config.fragments.entries) {
      if (_allFragments.containsKey(entry.key)) {
        throw MapperLoadException(
          'duplicate <sql> id across mappers: ${entry.key}',
          path: source,
        );
      }
      _allFragments[entry.key] = entry.value;
    }
