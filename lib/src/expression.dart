...
class _Lexer {
  ...
  _Token _readNumber() {
    ...
    final Object value;
    try {
      value = text.contains('.') ? double.parse(text) : int.parse(text);
    } on FormatException {
      throw ExpressionException('malformed number: "$text"', source);
    }
...
class ExpressionEvaluator {
  ...
  /// How many expressions are currently cached.
  ...
  static Object? evaluate(String expression, Map<String, dynamic> params) {
    // <foreach> rewrites #{item.prop} to #{__frch_item_0.prop}, so every row
    // of a bulk insert would add a permanent cache entry.
    if (expression.startsWith('__frch_')) return _parse(expression).eval(params);
  ...
  static _Node _parse(String expression) {
