...
First release on pub.dev. Three small correctness fixes found in review.

### Fixed — `timeout="0"` timed out immediately

MyBatis passes `timeout` to JDBC's `setQueryTimeout`, where `0` means *no
limit*. This port turned it into `Duration(seconds: 0)`, so every execution of
that statement failed at once. `0` (and any negative value) now means no
timeout, and it overrides the global default rather than falling back to it.

### Fixed — a timeout did not say which mapper it came from

`SqlTimeoutException.statementId` was documented as `namespace.id` but carried
the bare id, so the message read `statement "selectList" did not finish` with
no way to tell which mapper that was.

### Fixed — a rejected mapper left part of itself registered

`loadMapper` registered statements one at a time and threw on the first
duplicate id, after the preceding statements of that same mapper were already
in. An application that caught the error and carried on kept a half-loaded
mapper. Everything is validated before anything is registered.

### Install

The READMEs now install from pub.dev; the git instructions remain as an
alternative.
