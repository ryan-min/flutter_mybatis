...
''');
  ...
  test('timeout="0" means no limit, as in JDBC', () {
    final f = SqlSessionFactory(db)
      ..loadMapperFromString('''
<mapper namespace="Tmo">
  <select id="unlimited" timeout="0">SELECT 1</select>
  <select id="oneSecond" timeout="1">SELECT 1</select>
  <select id="unset">SELECT 1</select>
</mapper>
''');
    final stmts = XmlSqlParser.parse('''
<mapper namespace="Tmo">
  <select id="unlimited" timeout="0">SELECT 1</select>
  <select id="oneSecond" timeout="1">SELECT 1</select>
  <select id="unset">SELECT 1</select>
...
''').statements;
    expect(f, isNotNull);
    expect(stmts['Tmo.unlimited']!.timeoutDuration, isNull);
    expect(stmts['Tmo.oneSecond']!.timeoutDuration, const Duration(seconds: 1));

    // An explicit 0 overrides the global default rather than falling back.
    MybatisConfig.defaultStatementTimeout = const Duration(seconds: 5);
    expect(stmts['Tmo.unlimited']!.timeoutDuration, isNull);
    expect(stmts['Tmo.unset']!.timeoutDuration, const Duration(seconds: 5));
  });

  test('a timeout names the statement with its namespace', () {
    final e = SqlTimeoutException('T.upd', const Duration(seconds: 1));
    expect(e.message, contains('T.upd'));
  });

  test('a rejected mapper leaves no partial state behind', () {
    final f = SqlSessionFactory(db)
      ..loadMapperFromString('''
<mapper namespace="A">
...
''');
    // The second mapper collides on A.dup only after its other statements
    // would have been registered. Nothing of it may survive the rejection.
    expect(
      () => f.loadMapperFromString('''
<mapper namespace="A">
  <select id="fresh">SELECT 2</select>
  <select id="dup">SELECT 3</select>
</mapper>
'''),
      throwsA(isA<MapperLoadException>()),
    );
    final s = f.openSession();
    expect(() => s.selectList('A.fresh', {}), throwsA(isA<Exception>()));
  });
}
