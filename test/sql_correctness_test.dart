...
''';
  ...
  group('<where> conjunction stripping', () {
    test('AND on its own line is stripped, as in MyBatis', () {
      final sql = buildSql('''
        <select id="s">
          SELECT * FROM P
          <where>
    ...
    test('a column starting with OR/AND keeps its name', () {
      final sql = buildSql('''
        <select id="s">
          SELECT * FROM P
          <where>ORDER_ID = #{id}</where>
        </select>
      ''', {'id': 1});
      expect(sql, 'SELECT * FROM P WHERE ORDER_ID = ?');
    });

    test('an explicit <trim> keeps MyBatis semantics: overrides are literal',
        () {
      // MyBatis matches the override text verbatim, so an override written
      // as 'AND ' does not match an AND followed by a line break. That is
      // parity with MyBatis, not an oversight.
      final sql = buildSql('''
        <select id="s">
          SELECT * FROM P
          <trim prefix="WHERE" prefixOverrides="AND |OR ">
      ...
      ''', {'id': 1});
      expect(sql, 'SELECT * FROM P WHERE AND ID = ?');
    });
  });
}
