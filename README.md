...
| `#{param}` | prepared statement binding, including nested paths (`#{user.id}`, `#{ids[0]}`) |
| `${param}` | raw string substitution, same path rules |
...
`1.0.2` means the public API above is stable. Everything on the roadmap
